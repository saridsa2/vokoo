...
export function useBackButtonHandler(canExit: (routeName: string) => boolean) {
  ...
  useEffect(() => {
    ...
    const onBackPress = () => {
      ...
      // grab the current route. `getRootState` is typed as possibly undefined —
      // it is, briefly, between the container mounting and the first navigator
      // registering — and with no state there is no route to exit from.
