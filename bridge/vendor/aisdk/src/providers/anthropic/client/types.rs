...
// ---- VOKOO OVERRIDE ----
// `#[serde(default)]` on the container, so a missing usage field is zero
// rather than a parse failure.
//
// MiniMax serves the Anthropic Messages API shape and omits `cache_creation`,
// `service_tier` and others. Without this the request succeeds with HTTP 200
// and the reply is rejected here — reported as `missing field cache_creation`,
// which reads as our bug and is a provider that simply has no such concept.
//
// Costs nothing for Anthropic proper, which sends every field.
#[derive(Debug, Default, Clone, Serialize, Deserialize)]
#[serde(default)]
