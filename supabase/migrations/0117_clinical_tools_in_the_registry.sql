-- The clinical tools this product ships.
--
...
begin
  for r in
    select * from (values
    ('5908a9e5-542f-459e-a95a-08d66b5f79db'::uuid, 'normalise_unit', 'Convert a lab measurement from one unit to another, for example mg/dL to mmol/L. Mass-to-molar conversions need the analyte, because the factor is its molar mass. Returns converted: false with a reason when the conversion is not one this can make correctly — treat that as no answer, never as zero.', '{"type":"object","properties":{"value":{"type":"number","description":"The measured number, in `from` units."},"from":{"type":"string","description":"The unit as reported, e.g. ''mg/dL''."},"to":{"type":"string","description":"The unit wanted, e.g. ''mmol/L''."},"analyte":{"type":"string","description":"What was measured, lower case with underscores — glucose, creatinine, urea, cholesterol, triglyceride, bilirubin, calcium, uric_acid, albumin. Required when crossing between mass and molar units."}},"required":["value","from","to"]}'::jsonb, 'sha256:9a7f27775522a3674d9fa0f49b1f0e0f35a006ca48c6ccd9449832afc203bc09', 'import { defineTool } from "@vokoo/sdk";
