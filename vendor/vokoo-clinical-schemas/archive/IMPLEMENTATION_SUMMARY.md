...
已成功为 `/Users/zhangsan/kxb-website/open-vokoo/archive` 下的 **14个项目** 创建了完整的 Python + pyproject.toml 结构。
...
### 2. vokoo-healthkit-mapper
**功能**: 映射 Apple HealthKit 数据到 VoKoo schemas
...
**安装使用**:
```bash
cd vokoo-healthkit-mapper/language/python
pip install -e .
python examples.py
```
...
待实现核心功能的项目:
1. **vokoo-pdf-medical-parser** - PDF 医疗报告解析
2. **vokoo-medical-timeline** - 患者时间线构建
3. **vokoo-anomaly-flagger** - 数据质量异常检测
4. **vokoo-trend-detector** - 健康趋势分析
5. **vokoo-data-correlation** - 数据相关性分析
6. **vokoo-report-structurer-ai** - AI 报告结构化
7. **vokoo-fhir-lite** - FHIR 轻量映射
8. **vokoo-consent-model** - 同意管理模型
9. **vokoo-health-audit-log** - 审计日志
10. **vokoo-health-data-anonymizer** - 数据匿名化
11. **vokoo-radiation-dose-calc** - 放射剂量计算
...
3. **Schema 集成** - 所有项目都依赖和使用 vokoo 核心 schema
...
### 立即可用
1. **vokoo-lab-parser** - 可立即用于生产环境
2. **vokoo-healthkit-mapper** - 可立即用于生产环境
...
```toml
dependencies = [
    "vokoo-clinical-schemas>=0.1.0",  # 核心 schema
]
```
...
查看具体项目文档:
```bash
cd vokoo-{project-name}/language/python
cat README.md
```
