...
// VOKOO OVERRIDE — React Bits ships `cn`; this project's class utility is
// `cx` (it is `twMerge` directly). Aliased rather than renamed at every call
// site, so a re-install of this component is a one-line fix again.
...
void main() {
  ...
  // VOKOO OVERRIDE - carry the source's alpha into the particle.
  // Upstream samples rgb and drops a, so a particle born on a transparent
  // pixel is tinted black and drawn at full opacity: point a logo with a
  // transparent ground at this and you get a black square. Multiplying the
  // alpha through means the transparent region emits nothing visible, and the
  // logo works on any background with no flattened copy of the asset.
  // NOTE: no backticks in this comment - it lives inside a template literal.
  vec4 src = texture(uImage, coverUv(birthUv, uResolution.x / uResolution.y, uImageAspect));
  vTint = src.rgb;
...
void main() {
  ...
  // VOKOO OVERRIDE - same fix on the image drawn behind the particles.
  // Upstream forced every pixel to uOpacity, so a transparent ground was
  // painted as an opaque black rectangle.
