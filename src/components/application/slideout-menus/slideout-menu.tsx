...
interface SlideoutMenuProps extends Omit<AriaModalOverlayProps, "children">, RefAttributes<HTMLDivElement> {
    ...
    /**
     * Classes for the full-viewport backdrop.
     *
     * Separate from `className`, which styles the panel. They were previously
     * the same value: `className` was spread onto the overlay *and* applied to
     * the panel, so a width passed for the panel also capped the backdrop. A
     * `max-w-*` then shrank a `fixed inset-0` element to that width, and
     * `justify-end` placed the panel at the right edge of the shrunken box —
     * which is the left of the screen. The panel appeared on the wrong side and
     * the dimming covered only part of the page.
     */
    overlayClassName?: AriaModalOverlayProps["className"];
}

const Menu = ({ children, className, dialogClassName, overlayClassName, ...props }: SlideoutMenuProps) => {
    return (
