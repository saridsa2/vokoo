...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // AriaCalendarContext types onChange as accepting a concrete date (or an
    // array, for range calendars), which a plain setState dispatch does not
    // satisfy. Wrapping keeps the setter's behaviour and narrows the signature
    // to the single-date form this provider actually uses.
    return (
        <AriaCalendarContext.Provider
            value={{
                value,
                onChange: (next) => onChange((next as DateValue | null) ?? null),
                focusedValue,
                onFocusChange,
            }}
        >
